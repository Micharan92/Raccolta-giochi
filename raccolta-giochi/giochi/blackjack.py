...
def _css_tavolo():
    ...
    @media (max-width: 768px) {
        ...
        .bj-label { font-size: 0.85rem; }
        .bj-hand { gap: 6px; min-height: 78px; }
        .bj-card {
            width: 50px;
            height: 72px;
            padding: 4px;
        ...
        .bj-card-val-top, .bj-card-val-bot { font-size: 0.78rem; }
        .bj-card-suit-mid { font-size: 1.05rem; }
        .bj-card.back { font-size: 1.25rem; }
        .bj-score { font-size: 0.8rem; }
        .bj-dealer-msg { font-size: 0.85rem; }
    }
    </style>
    """
...
def _html_tabella_guida():
    valori = (
        ...
        ("J · Q · K", "10"),
        ("Asso", "11 o 1 (automatico)"),
    )
    azioni = (
        ("Pesca carta", "Chiedi un'altra carta al banco"),
        ("Fermati", "Concludi il tuo turno; gioca il banco"),
    )
    corpo_valori = "".join(f"<tr><td>{c}</td><td>{v}</td></tr>" for c, v in valori)
    corpo_azioni = "".join(
        f"<tr><td><strong>{a}</strong></td><td>{d}</td></tr>" for a, d in azioni
    )
    return (
        '<div class="br-score-wrap">'
        + '<div class="br-score-title">Regole e punteggi Blackjack</div>'
        + '<table class="br-score-table">'
        + '<thead><tr><th>Carta</th><th>Valore</th></tr></thead>'
        + f"<tbody>{corpo_valori}</tbody>"
        + "</table>"
        ...
        + "<strong>Obiettivo:</strong> avvicinarsi a 21 senza superarlo. "
        + "Superi 21: sconfitta immediata. Con <strong>Fermati</strong> il banco "
        + "scopre la carta coperta e pesca fino a 17 o più. "
        + "Vince chi ha il total più alto; stesso punteggio: pareggio."
        + "</div></div>"
    )
...
def render():
    ...
    if stato["fase"] == "giocatore":
        ...
        with col1:
            if st.button("🃏 Pesca carta", key="bj_carta", use_container_width=True):
                if stato["mazzo"]:
                ...
                st.rerun()
        with col2:
            if st.button("✋ Fermati", key="bj_ferma", use_container_width=True):
                stato["fase"] = "banco"
    ...
    if stato["fase"] == "fine":
        ...
        if st.button("🔄 Nuova partita", key="bj_reset", use_container_width=True):
            st.session_state[SESSION_KEY] = _nuova_partita()
            st.rerun()
        _render_guida()
    elif stato["fase"] == "banco":
        # Pulsante di recupero se il turno del banco si blocca
        if st.button("🔄 Nuova partita", key="bj_reset_recupero", use_container_width=True):
            st.session_state[SESSION_KEY] = _nuova_partita()
            st.rerun()
        _render_guida()
